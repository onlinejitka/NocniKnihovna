...
import { RotateCcw, Lightbulb, Maximize2, Minimize2, Sparkles, ShoppingBag } from 'lucide-react';
...
export default function Hra() {
    ...
    setFireflies((prevFireflies) => {
      ...
      const updated = prevFireflies.map((f) => {
        ...
        if (f.isCollected) {
          ...
          return { ...f, x: f.x + dx * 0.06, y: f.y + dy * 0.06 };
        }
  ...
  const toggleFullscreen = () => {
    ...
    if (!document.fullscreenElement) {
      gameContainerRef.current.requestFullscreen().then(() => setIsFullscreen(true)).catch((err) => console.error(err));
    } else {
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="flex justify-end px-2">
        <button onClick={toggleFullscreen} className="inline-flex items-center space-x-2 bg-slate-900/80 hover:bg-slate-800 border border-slate-800 text-slate-300 text-xs font-bold px-4 py-2 rounded-xl transition cursor-pointer shadow-md">
          ...
          <span>{isFullscreen ? 'Zmenšit okno' : 'Zobrazit na celou obrazovku'}</span>
        </button>
      </div>

      <div ref={gameContainerRef} className={`relative w-full aspect-[16/10] bg-[#030206] border border-slate-900 overflow-hidden select-none group transition-all duration-300 ${isFullscreen ? 'rounded-none border-none' : 'rounded-3xl shadow-2xl'}`}>
        ...
        {fireflies.map((f) => {
          ...
          return (
            <button key={f.id} onClick={() => handleFireflyClick(f.id)} disabled={f.isCollected} style={{ left: `${f.x}%`, top: `${f.y}%`, width: '44px', height: '44px', transform: 'translate(-50%, -50%)' }} className="absolute flex items-center justify-center bg-transparent border-none outline-none cursor-pointer z-10 p-0">
              <div style={{ width: `${f.size}px`, height: `${f.size}px` }} className={`rounded-full bg-amber-200 transition-all duration-300 ${f.isCollected ? 'shadow-[0_0_20px_#fbbf24,0_0_40px_#fbbf24] bg-white scale-125' : 'shadow-[0_0_10px_#fbbf24,0_0_25px_rgba(251,191,36,0.8)] animate-pulse'}`} />
            </button>
          );
        })}

        <div style={{ left: `${LANTERN_X}%`, top: `${LANTERN_Y}%`, transform: 'translate(-50%, -50%)' }} className="absolute w-20 h-28 flex flex-col items-center justify-end">
          ...
          <div className="w-14 h-20 bg-slate-900/90 border-2 border-slate-700 rounded-b-xl relative flex items-center justify-center overflow-hidden">
            <div style={{ opacity: insideCount / TOTAL_FIREFLIES, transform: `scale(${1 + (insideCount * 0.2)})` }} className="absolute w-10 h-10 bg-gradient-to-br from-amber-300 to-orange-500 rounded-full filter blur-md transition-all duration-500 shadow-[0_0_30px_rgba(251,191,36,0.6)]" />
            <Lightbulb size={24} className={`relative z-10 transition-colors duration-500 ${insideCount > 0 ? 'text-amber-200 drop-shadow-[0_0_8px_rgba(251,191,36,1)]' : 'text-slate-700'}`} />
          </div>
          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-2 select-none">{insideCount} / {TOTAL_FIREFLIES}</span>
        </div>
        ...
        {gameCompleted && (
          <div className="absolute inset-0 bg-slate-950/95 backdrop-blur-sm flex flex-col items-center justify-center text-center p-6 animate-fade-in z-20">
            ...
            <h3 className="text-xl md:text-2xl font-serif font-bold text-amber-300 max-w-md leading-snug">Lucerna jasně září...</h3>
            <p className="text-slate-300 text-sm md:text-base max-w-md mt-2 leading-relaxed px-4">Všechny světlušky jsou bezpečně v pelíšku a lucerna Vám bude celou noc hlídat klidné sny. Nyní je čas zavřít očička i u Vás doma.</p>
            <p className="text-amber-400/80 text-xs italic mt-2 font-medium">Přejeme Vám kouzelnou dobrou noc.</p>
            <button onClick={initGame} className="mt-6 inline-flex items-center space-x-2 bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-300 text-xs font-bold px-4 py-2 rounded-xl transition cursor-pointer"><RotateCcw size={12} /> <span>Rozsvítit znovu</span></button>
          </div>
        )}
      </div>

      {/* INTEGROVANÝ AFFILIATE BANNER NA HRÁČKY (BEZPEČNĚ UVNITŘ) */}
